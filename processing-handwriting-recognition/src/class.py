...
dataSet = np.loadtxt(open("data/test_train.csv"), delimiter=",", dtype=np.uint8)
label = np.loadtxt(open("data/test_label.csv"), delimiter=",")
# format train and test
X_train, X_test, Y_train, Y_test = train_test_split(dataSet, label, test_size=0.2, random_state=0)
X_test = np.loadtxt('data/validation_feature.csv', delimiter=',', dtype=np.uint8)
Y_test = np.loadtxt('data/validation_label.csv', delimiter=',', dtype=np.uint8)
